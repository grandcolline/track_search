...
//! | [ユースケース層](./usecase) | ビジネスフロー/アプリケーションロジックを定義する層。ポートも持つ。|
//! | [アプリケーション層](./application) | ユーザとユースケースをつなぐ層。ユースケースの事項を行う。 |
...
//!             DRIVER
//! ```
//! * アプリケーション層とアダプター層はそれぞれドライバー層から参照され、ユースケース層に依存するが、互いには依存し合わない。
